...
vault kv put -mount=security-codedesignplus-keyvalue ms-roles `
...
vault kv get -mount=security-codedesignplus-keyvalue ms-roles

# --- 7. Writing database configuration ---
Write-Host $newlines
Write-Host "7. Writing database configuration..." -ForegroundColor Blue
vault write security-codedesignplus-database/config/db-ms-roles `
    plugin_name=mongodb-database-plugin `
    allowed_roles="ms-roles-mongo-role" `
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" `
    username="admin" `
    password="password"

vault write security-codedesignplus-database/roles/ms-roles-mongo-role `
    db_name=db-ms-roles `
    creation_statements="{ """db""": """admin""", """roles""": [{ """role""": """readWrite""", """db""": """db-ms-roles""" }] }" `
    default_ttl="1h" `
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-roles-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-roles-rabbitmq-role `
    vhosts="{"""/""":{"""write""": """.*""", """read""": """.*""", """configure""": """.*"""}}"

vault read security-codedesignplus-rabbitmq/creds/ms-roles-rabbitmq-role