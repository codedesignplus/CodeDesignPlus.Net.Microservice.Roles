...
5. Run the desired entry point:
   ...
   - For gRPC:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Roles.gRpc
      ```

   - For Worker:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Roles.AsyncWorker
      ```
...
1. Build the Docker image using the Dockerfile in the REST API entry point:
   ...
   docker run -d -p 5000:5000 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-roles-rest ms-roles-rest
   ```

2. Build the Docker image using the Dockerfile in the gRPC entry point:
   ```bash
   docker build -t ms-roles-grpc . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Roles.gRpc/Dockerfile

   docker run -d -p 5001:5001 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-roles-grpc ms-roles-grpc
   ```

3. Build the Docker image using the Dockerfile in the Worker entry point:
   ```bash
   docker build -t ms-roles-worker . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Roles.AsyncWorker/Dockerfile

   docker run -d -p 5002:5002 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-roles-worker ms-roles-worker
   ```
